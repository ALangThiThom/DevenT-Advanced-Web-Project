                    ...
                    <tr key={event.id} className={styles.tableRow}>
                      ...
                      <td>
                        <div style={{ display: "flex", alignItems: "center", gap: "0.75rem" }}>
                          <div style={{ width: "32px", height: "32px", borderRadius: "4px", backgroundColor: "#E5E7EB", overflow: "hidden", display: "flex", alignItems: "center", justifyContent: "center" }}>
                      ...
                      <td>
                        <span className={
                          event.status === 'published' ? styles.badgePublished : 
                          event.status === 'cancelled' ? styles.badgeCancelled : 
                          styles.badgeDraft
                        }>
                          ...
                          <div style={{ display: "flex", flexDirection: "column", gap: "0.25rem", width: "128px" }}>
                            ...
                            <div style={{ height: "6px", width: "100%", backgroundColor: "#F3F4F6", borderRadius: "9999px" }}>
                              <div 
                                style={{ 
                                  height: "100%", 
                                  backgroundColor: getProgressColor(event.status), 
                                  borderRadius: "9999px", 
                      ...
                      <td style={{ textAlign: "right" }}>
                        <div className={styles.actionGroup}>
                          <button className={styles.actionBtn} title="Edit Event">
      ...
      {!loading && eventsData?.data?.length > 0 && (
        <div style={{ padding: "1rem 1.5rem", borderTop: "1px solid var(--border-main)", display: "flex", alignItems: "center", justifyContent: "space-between", backgroundColor: "#F9FAFB" }}>
          <span style={{ fontSize: "0.75rem", fontWeight: "500", color: "var(--on-surface-variant)" }}>
            Showing {eventsData.data.length} of {eventsData.total || eventsData.data.length} events
          </span>
          <div style={{ display: "flex", gap: "0.5rem" }}>
            <button className={styles.secondaryBtn} style={{ padding: "0.375rem 0.75rem", fontSize: "0.75rem", opacity: 0.5, cursor: "not-allowed" }} disabled>
              Previous
            </button>
            <button className={styles.secondaryBtn} style={{ padding: "0.375rem 0.75rem", fontSize: "0.75rem" }}>
              Next
            </button>
          </div>
        </div>
